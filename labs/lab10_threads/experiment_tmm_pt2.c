...
int main(int argc, char *argv[])
{
    ...
    double cpuTimeTotal;
    struct timespec start_cpu,start_elapsed,end_cpu,end_elapsed;
    int rc;
     ...
     */
    rc = clock_gettime(CLOCK_REALTIME,&start_elapsed);
    assert(rc==0);
    rc = clock_gettime(CLOCK_PROCESS_CPUTIME_ID,&start_cpu);
    assert(rc==0);
     ...
     * (`CLOCK_PROCESS_CPUTIME_ID`) clocks.
     */
    rc = clock_gettime(CLOCK_REALTIME,&end_elapsed);
    assert(rc==0);
    rc = clock_gettime(CLOCK_PROCESS_CPUTIME_ID,&end_cpu);
    assert(rc==0);
    ...
    //  *     print out the cpuTime according to matMulThreadStats[0]
        printf("%*s: %lf\n",colonIndent,"CPU Time",matMulThreadStats[0].cpuTime);
    }
    //  * otherwise
    else {
    ...
    //  *         print out the cpuTime used by thread i
            printf("%*s %3d: %lf\n",colonIndent-4,"CPU Time", i, matMulThreadStats[i].cpuTime);
    ...
    //  *         print out the number of rows multiplied by thread i
            printf("%*s %3d: %d\n",colonIndent-4,"Rows Multiplied", i, matMulThreadStats[i].nRowsDone);
        }
    //  *     print out the thread cpu time sum
        printf("%*s: %lf\n",colonIndent,"Total CPU Time",cpuTimeTotal);
    }
     ...
     */
    printf("%*s: %lf\n",colonIndent,"Elapsed",tspecDiff(start_elapsed,end_elapsed));
    printf("%*s: %lf\n",colonIndent,"CPU",tspecDiff(start_cpu,end_cpu));
    /*
     * ASSIGNMENT
     *
